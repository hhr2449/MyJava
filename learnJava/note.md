...
**作用**：提供各种实用工具类和集合框架（Collections Framework）。

**常见类**：

- `ArrayList`, `LinkedList`, `HashMap`, `HashSet` 等集合类

- `Date`, `Calendar`, `LocalDateTime`（在 `java.time` 包中也有更新版本）等日期时间类（注意：部分日期类已迁移到 `java.time` 包）

- `Random` 随机数生成器

  ==注意，在lang包中的Math类也提供了random方法，但是和Random类不同，random方法只能够生成简单的随机数==

- `Scanner` 用于从输入流读取数据

- `Collections` 工具类，提供对集合的操作方法

**用途**：处理集合、日期、随机数、输入扫描等通用功能。

------

### 2. `java.io`

**作用**：提供用于输入/输出（I/O）操作的类和接口，支持文件读写、流操作等。
...
- `FileInputStream`, `FileOutputStream`, `FileReader`, `FileWriter` 等具体实现类

**用途**：文件操作、网络通信中的数据流处理等。

------

### 3. `java.awt`

**作用**：抽象窗口工具包（Abstract Window Toolkit），用于创建图形用户界面（GUI）和进行图形绘制。

**常见类**：

- `Frame` 窗口
- `Button`, `Label`, `TextField` 等组件
- `Graphics` 用于绘图
- `Color`, `Font` 等图形属性类

**用途**：开发桌面 GUI 应用程序。不过现在更推荐使用 `javax.swing` 或 JavaFX 来开发更现代的 GUI。

------

### 4. `java.lang`

**作用**：Java 语言的核心包，包含了 Java 编程中最基础、最常用的类。这个包是**默认导入**的，也就是说你不需要显式地写 `import java.lang.*;` 就可以直接使用其中的类。

**常见类**：

- `Object` 所有类的超类
- `String`, `StringBuilder`, `StringBuffer` 字符串处理类
- `Integer`, `Double` 等包装类（wrapper classes）
- `System` 提供系统相关功能，如输入输出、环境变量等
- `Math` 数学计算工具类
- `Thread` 线程类
- `Exception`, `RuntimeException` 等异常类

**用途**：Java 编程的基础，几乎每个 Java 程序都会用到这个包中的类。
...
1. java的package语句必须放在java程序的最开始

2. java的局部变量（方法变量）

   * 局部变量只在声明它的方法、代码块内有效

   * 局部变量在方法执行到对应语句时创建，方法结束时销毁，每次调用方法都会创建一个新的

   * 方法变量必须初始化才能使用

     原因在于java会自动初始化成员变量，但是不会自动初始化局部变量
   ...
   * 方法变量可以覆盖类的成员变量，但是不允许在外层和内层同时定义同名变量
...
3. 左移运算符和右移运算符
   ...
   1. ##  **左移运算符（`<<`）**
      ...
      - 将二进制数 **向左移动指定位数**，低位补 `0`。
      - 相当于 **乘以 2\*n\***（`n` 是移动位数）

   2. ## **带符号右移运算符（`>>`）**

      - 将二进制数 **向右移动指定位数**，**高位补符号位**（正数补 `0`，负数补 `1`）。
      - 相当于 **除以 2\*n\* 并向下取整**（对负数有效）。

   3. ## **无符号右移运算符（`>>>`）**

      - 将二进制数 **向右移动指定位数**，**高位补 `0`**（无论正负）。
      - 负数右移后会变成正数。

4. java中的整数和浮点数

   Java中的整数默认为int类型，浮点数默认为double类型

   常见错误：

   1. 整数位数问题：long l = 1000000000000;这里虽然l是整数类型，但是1000000000000默认为int类型，导致溢出，应该使用1000000000000L

   2. 包装类构造问题：包装类构造可以使用两种构造方式

      * Long l = 100L;
      * Long l = new Long(100)

      对于上面的构造方式一定要保证类型符合（涉及自动装箱），如果直接使用100会报错

      下面的构造方式虽然不会报错，但是Long(int)的重载已被弃用，不提倡这种用法

5. 接口和超类中有同名同参方法怎么办

   1. 超类优先原则

      当超类中有这个方法的实现，无论接口中是否有该方法的实现，都会采用超类中的实现，即使使用接口的引用调用方法也一样

      ```java
      interface MyInterface {
          default void print() { // 接口默认方法
              System.out.println("Interface's print");
          }
      }
      
      class SuperClass {
          public void print() { // 超类方法
              System.out.println("SuperClass's print");
          }
      }
      ...
      public class Test {
          public static void main(String[] args) {
              ...
              sub.print(); // 输出: SuperClass's print
      
              // 即使通过接口引用调用，仍然使用超类方法！
              MyInterface obj = new SubClass();
              obj.print(); // 输出: SuperClass's print
          }
      }
      ```
   ...
   2. 当超类中没有默认实现（是抽象类）时
      ...
      如果接口中有默认实现，那么可以采用接口中的默认实现，如果没有，必须重写

   3. 当多个接口冲突时

      必须重写

6. public类的主方法是静态方法，所以在主方法中只能够直接访问public类的静态成员变量，不能访问非静态成员，但是可以在主方法中创建public类的实例，用实例对象去访问
...
7. 成员变量的初始值
   ...
   |    `int`     |    `0`     |      32 位有符号整数       |
   |    `long`    |    `0L`    |      64 位有符号整数       |
   |   `float`    |   `0.0f`   |     32 位单精度浮点数      |
   |   `double`   |   `0.0d`   |     64 位双精度浮点数      |
   |    `char`    | `'\u0000'` | Unicode 空字符（显示为空） |
   |  `boolean`   |  `false`   |  布尔值（`true`/`false`）  |
   |   类的引用   |    null    |           空引用           |

8. 变量的比较（使用==）

   同类之间才能比较，不同类之间无法比较（比如char[]和String）

   基本类型比较的是值是否相同

   引用类型比较的是引用的地址是否相同（包括包装类）

   特殊情况：

   1. String s = "hello"这样直接赋值的字符串常量会复用对象池中的对象，所以只要内容相同就会引用同一个对象

   2. Boolean b = true这样定义的boolean类型变量是单例的
...
9. 短路型的逻辑与和或`&&,||`在前面的逻辑语句已经可以判定时，后面的语句就不会再执行了，即使后面语句有运行时错误，只要前面的语句正确就不会得到执行，就不会报错
...
10. java中的数组长度获取
    ...
    数组有一个属性为length，可以使用arr.length来获取，数组没有length()方法和size()方法，length()方法是集合类型的方法
...
* char是16位unicode，注意char是无符号的，==取值范围是0~$2^{16}-1$,默认值是/u0000(表示空字符)==
...
在使用集合框架的时候，可以创建包装类的集合（因为集合不能接受基本类型），然后直接将对应的基本类型加入
...
![image-20250703192922591](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250703192922591.png)

注意，>>是右移运算符，向右移位，空位应该补符号位，如果符号位是1就补1，是0就补0，运算后相当于除2^n,且保留符号

<<是左移运算符，空位一定补0，相当于乘2^n

`>>>`是无符号右移，空位一定补0，相当于除2^n且取绝对值

注意&，|除了位运算符，还可以充当非短路的逻辑与和或
...
```java
    public static void main(String[] args) {
        ...
        System.out.println(s==new String("hello"));
...
![image-20250703111915688](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250703111915688.png)

1. 对于使用`String s = "contents"`进行构造的字符串，内容相同即是引用相同
2. 对于使用`String s = new String("contents")`和`String s = new String(char[] c)`进行构造的字符串，内容相同，引用不同
3. String类和char[]类，内容不同，引用不同
...
==注意：如果要声明与父类中的方法同名且同参数列表的方法，必须实现覆盖，也就是不能再子类中定义和父类中的某个方法同名，同参数列表，但是返回值不同的方法，也不能父类非静态，子类静态==
...
类方法不会发生多态，在编译期间就根据引用的类型来确定了调用的函数，所以类方法调用跟实际指向的对象类型无关，只和引用变量的类型有关
...
1. 成员内部类：可以访问类中所有成员变量
   ...
   public class OuterClass {
       ...
       void createInner() {
           InnerClass inner = new InnerClass(); // 内部直接创建
       }
   }
...
3. 局部内部类：定义在方法里面的类，作用域仅限于方法中，可以访问所有成员变量和方法中的final局部变量和参数，不能访问非final的局部变量和参数
   ...
   public class OuterClass {
       void createLocalInner() {
           ...
           LocalInnerClass local = new LocalInnerClass(); // 仅在此方法内可创建
       }
   }
   ```
...
增强了类型定义系统，为集合框架提供了编译时的类型检查，避免不必要的类型转换，同时避免运行时的类型错误

相当于告诉编译器元素类型，由编译器来进行类型检查与转换

```java
public static void main(String argv[]){
    List a = new ArrayList();
    //a.add(new Double(1.0));   在没有指定类型的ArrayList中可以放入任意类型的对象，这样在进行强制类型转换是可能会有运行时错误
    a.add(new Integer(1));
    a.add(new Integer(2));
    //Integer j = a.get(0);   ArrayList a中的对象类型是Object,必须使用强制类型转换
    Integer j = (Integer)a.get(0);

    List<Integer> l = new ArrayList<Integer>();
    //l.add(new Double(0));   编译时会进行类型检查，避免强制转换导致的类型错误
    l.add(new Integer(1));
    l.add(new Integer(2));
    Integer i = l.get(0);
...
泛型参数通常使用一个大写字母表示，以区分泛型参数和普通的类型名
...
3. 插入强制类型转换
   ...
   其实泛型做的事情和没有泛型的时候编程的方式是类似的，都是由基本类型进行强制类型转换成为目标类型，只是泛型将事情交给编译器来做，并且加入了转换
   
   ==注意泛型参数不能使用基本类型，因为泛型参数需要进行类型擦除，而基本数据类型无法进行，可以使用基本类型的包装类==
   
   所有使用不同实际类型参数的泛型类使用同一个类定义，泛型参数不在运行时出现
   
   ```java
   List<Double> d = new ArrayList<Double>();
   List<String> s = new ArrayList<String>();
   System.out.println(s.getClass() == d.getClass() );//输出true，说明使用不同泛型参数的泛型类是同一个类
   ```
...
泛型类之间没有继承关系，也就是Collection<Object>和Collection<String>之间没有继承关系，即使泛型参数之间有继承关系

示例

```java
public class ObParm{
    static void print(Collection<Object> c) {
        for(Object o : c) {
    ...
    public static void main(String argv[]){
        ...
        l.add("two");
        l.add("three");
        //print(l);  报错，因为Collection<Object>不是Collection<String>的超类，所以传入l是不对的
    }
}
```
...
Collection<?>表示任意泛型参数的Collection
...
![image-20250704103017549](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250704103017549.png)

使用泛型参数，使print函数可以接受所有泛型参数的Collection

`<? extends ClassName>`用来表示ClassName本身及其子类

```java
public class ObParm{
    static void print(Collection<? extends Collection> c) {
        for(Object o : c) {
            System.out.println(o);
        }
    }
...
//        l3.add(new ValHold());
//        print(l3);
        List<ArrayList>l4 = new ArrayList<ArrayList>();
...
ArrayList是Collection的子类，可以传入，其他的不可以传入

#### 可变参数

允许方法中接受不定数目个参数

语法：dataType... parameterName，这里的parameterName会被当作数组来处理，可以用于接受多个参数

注意可变参数一定要放在参数表的最后面，而且只能有一个

```java
public class ObParm{
    public char a;
    public static void main(String argv[]){
        ObParm ob = new ObParm();
        ob.amethod(1,2,3,4);
        ob.amethod(2,3,4,5,6);
        ob.amethod(3,4,5,6,7,8);
        ob.amethod(4,5,6,7,8,9,10);

    }
    public void amethod(int a, int ... args) {
        for(int n : args) {
            System.out.print(n);
        }
        System.out.println();
...
将其他包中的类的静态函数给导入，这样可以直接使用函数名来进行调用，不需要使用类名

注意只有不同包才能使用import，同一个包不能使用
...
2. 将异常向上抛出（可能是系统的运行时异常，也可以是人为主动抛出异常）给其调用者方法（可以声明抛出异常但是不处理异常，将异常交给调用者处理，知道有合适的方法将该异常捕获）
...
![image-20250702105233067](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702105233067.png)
...
解决跨平台问题：不同操作系统中文件的分隔符不同(比如`\`和`/`)，java在拼接文件路径的时候会避免使用具体的分隔符号，而是使用静态常量`pathSeparator`和`separator`，然后jvm会根据具体的操作系统来输出对应的分隔符
...
一般在list(filter)和listFiles(filter)中使用，并且一般会创建匿名对象来作为参数

##### 综合小工具

使用以上知识制作的一个可以指定删除文件夹中满足条件的文件和文件夹的工具
...
public class ReadAndWrite {
    public static void main(String[] args) {
        ...
        //构造方法可能会产生FileNotFoundException
        catch (FileNotFoundException e) {
            System.out.println("文件不存在");
            e.printStackTrace();
        }
        //fis.read()可能会产生IOException
        catch (IOException e) {
...
![image-20250702091857931](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702091857931.png)

![image-20250702092043065](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702092043065.png)

RandomAccessFile类允许对文件进行随机存取（可以将文件指针跳转到各个位置读取）

**文件指针操作**：

|          方法           |                             作用                             |
| :---------------------: | :----------------------------------------------------------: |
| `long getFilePointer()` |    返回当前指针位置（从文件开头到当前位置的字节偏移量）。    |
|  `void seek(long pos)`  | 移动指针到指定位置（`pos` 是字节偏移量，注意是以0为参考点，不是当前位置）。 |
|     `long length()`     |                  返回文件总长度（字节数）。                  |

**读写操作**：

|             方法             |                         作用                          |
| :--------------------------: | :---------------------------------------------------: |
|         `int read()`         |   读取 1 个字节（返回 `int` 值，-1 表示文件结束）。   |
|     `int read(byte[] b)`     |      读取数据到字节数组，返回实际读取的字节数。       |
| `readInt()` / `readDouble()` | 直接读取 `int`、`double` 等基本类型（自动转换字节）。 |
|        `write(int b)`        |              写入 1 个字节（低 8 位）。               |
|      `writeInt(int v)`       |             写入一个 `int` 值（4 字节）。             |
...
![image-20250702094535795](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702094535795.png)

![image-20250702092358622](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702092358622.png)

writer:

![image-20250702092509327](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702092509327.png)

![image-20250702092455274](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702092455274.png)
...
这里使用了文件字符流FileWriter和FileReader，字符流可以直接写入字符串和写入字符数组，注意和字节流的区别：字节流使用Byte数组进行读写，而字符流使用char数组，还可以使用String

==缓冲问题==：缓冲流存在缓冲区，写入数据会存入缓冲区，等到缓冲区满了或是手动调用flush或close才会将数据写入文件

FileWriter不是缓冲流，可能由于本地内存或是JVM的缓冲机制，在上述代码中如果不调用close或是使用try-with-resource方式则会导致无法写入

==try-with-resource方法==：在try后跟的括号内构造文件流，这样try结束之后就会自动进行文件流的关闭，能够更加有效的进行流的管理
...
2. OutputStreamWriter类：将字符流转化为字节流输出
   ...
   文件存储(字节序列) → 字节流读取(原始字节) → 内存缓冲区 → 字符流解码(按编码规则) → 程序可读字符

   ==字节流仅用于读取文件中的原始字节序列，不关心其中的含义，不做任何的解释和转换。通过字节流将文件内容读取到内存中后，字符流再将读取到的东西按照对应的编码进行解析，然后输出对应的字符==

   

   优势：可以指定编码方式

   ![image-20250702094245283](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702094245283.png)

   ![image-20250702094331800](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702094331800.png)

   字节流将文件中的内容以字节的形式读到内存中，编码转换流再从内存中读取字节流的读取结果
...
![](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/屏幕截图 2025-07-02 154814.png)

记事本使用的是utf-8编码，所以写入gbk编码的数据会出错
...
![image-20250702155615812](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702155615812.png)

使用utf-8进行读取，得到乱码，使用gbk进行读取，正常

##### 缓存流

![image-20250702095148394](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702095148394.png)

![image-20250702095158559](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702095158559.png)

接受一个字符流，额外提供缓存功能

字符流将数据读取到内存中，缓冲流再从内存中进行读取



- **读操作**：一次性读取多个字节到缓冲区，后续读取直接从缓冲区获取
- **写操作**：先将数据写入缓冲区，缓冲区满后一次性写入物理存储
...
![image-20250702161550783](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702161550783.png)

##### 分层套接使用

可以使用如：字节流-->转换流-->缓冲流  的套接使用方式

字节流负责数据的传输（字节序列），字节流传输的数据进入转换流进行字符编码处理（字符序列），缓冲流提供缓冲区用来提供I/O优化，同时提供多样的读取功能

```java
new BufferedReader(          // 提供行读取功能
    new InputStreamReader(   // 处理字符编码
    new FileInputStream(     // 连接数据源
    "data.txt")))
```

其实是一种装饰器模式，在字节流的外部装上一层装饰器用于字节续流解析，形成转换流

字符流的外部装上一层装饰器，增添缓冲区，形成缓冲流



#### 过滤流

基于装饰器模式，可以在一个流的基础上套上一个过滤流来提供额外的处理能力，前面提到的缓冲流就是过滤流，给一般的字符流赋予缓冲的功能

![image-20250702162312663](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702162312663.png)

#### 键盘输入

核心：**System.in**：这是一个InputStream类的对象，由Jvm进行特殊初始化，与键盘设备进行关联，实现了输入操作

但是System.in只能以字节的方式进行读入，无法处理字符和字符串，而且没有缓冲区，具有局限性，所以使用的时候一般会对他进行包装

比较经典的两个方案是：

1. Scanner(System.in)使用Scanner类进行包装

   ![image-20250702164028665](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702164028665.png)

2. new BufferedReader(new InputStreamReader(System.in)))使用BufferReader和InputStreamReader进行包装

   **提供的方法**：

   - `read()`：读取单个字符
   - `read(char[] cbuf)`：读取到字符数组
   - `readLine()`：读取整行字符串（唯一的高阶方法）

* Scanner包装方案可以按照空格分词，可以直接读取int,double等数据类型，可以读取整行，方法更加多样，但是效率略低于另一个包装方案

* new BufferedReader(new InputStreamReader(System.in)))包装方案效率更高，但是只能处理字符串
...
```java
//使用BufferReader和InputStreamReader进行包装
...
public class KeyboradInput {
    public static void main(String[] args) {
        try {
            ...
            //使用System.in来构造InputStreamReader
            ir = new InputStreamReader(System.in);
            //使用ir来构造BufferReader
            ib = new BufferedReader(ir);
            System.out.println("请输入内容：");
            //使用BufferReader的readLine方法进行读取
...
```java
//使用System.in的read方法直接读取，但是此时只能够读取一个字节，而且不能读中文
try {
...
![image-20250702170329875](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702170329875.png)



#### 串行化（或称序列化）和对象流

实现Serialization接口实现串行化

该接口里面是空的，不用实现任何方法，作用是用于标示该类可进行序列化

只能保存非静态成员变量，静态成员变量是属于类的，一个对象保存了可能会被另一个变量改变，保存没有意义

**序列化**：将原本内存中的对象状态变成可传输的或可储存的（将其变成一个字节序列）过程称为序列化

**反序列化**：将之前的字节序列恢复成对象的过程称为反序列化

经过序列化和反序列化之后的对象的内容完全一致，但是对象本身存储的位置不一样

**序列化的应用**：

1. 将对象的状态存储到文件或数据库中，以便之后使用
2. 通过网络传输对象

**注意事项**：

1. 当父类实现了Serializable接口，子类自动实现序列化
2. 当一个对象的实例变量引用了另一个对象，当序列化这个对象时同时会序列化其引用的对象（要求引用的对象也要实现Serializable接口），经过序列化和反序列化后，引用的对象的内容不变，但是存储的地址变了
3. 序列化只会保存实例变量，静态变量和方法不会保留

![img](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/02b9825c78ca859879c3b05ef41b9a1a.png)

总结如图，将一个对象序列化为字节序列，通过文件，数据库存储，或是通过网络传输，当反序列化时，读取字节序列，将其恢复为原本的对象

**实现序列化**

使用ObjectInputStream和ObjectOutputStream来进行序列化，这两个类对象使用字节流进行初始化

调用writeObject进行写入，使用readObject进行读取
...
class Student implements Serializable {
    ...
    transient int id;
    String address;
    //里面有一个引用变量a，这里的A类也实现了接口，可以一起序列化
    A a;
...
public class ObjectStream {
    public static void main(String[] args) {
        ...
        try {
            ...
            //输出结果发现序列化和反序列化之后stu的序列化变量内容一样，但是未经序列化的变量id不同
            //引用变量a的地址发生了变化，但是a的内容不变
...
输出结果：![image-20250702200512806](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702200512806.png)

**序列化的问题**：

1. 安全性：序列化时，类中的final,private等访问控制符会被忽略，这些敏感数据被转化为字节序列之后很容易被破解
2. 资源问题：有时候希望对对象的创建进行控制（比如单例模式），如果将其序列化，则会突破这种控制，可以通过序列化的字节序列来随意创建

**transient关键字**：在变量前标注transient关键字，则变量不会被序列化，可以保护一些敏感数据，同时对于一些瞬时化的变量（比如Thread对象，FileInputStream对象）应该使用transient关键字标注
...
==java中的集合类型可以混合放入不同类型的对象（当成object），但是推荐优先使用同类型，这样更安全==
...
1. ArrayList:相当于vector,是一个长度可变的数组，其中的每个元素都是对象

   ArrayList中的元素是按照添加的顺序存放的，允许重复
   ...
   * AyyayList里面实现了接口List，所以可以直接使用List来声明

   * ArrayList可以不指定放入对象的类型，可以混合存放
...
1. HashMap:key无序
   ...
   HashMap中的key是不重复的，如果放入一个已经有的key的键值对，则后放入的会覆盖新放入的

   HashMap中的key和value都可以设置为null
   ...
   hashmap.remove(1);
   System.out.println("Map: " + hashmap);
   //keySet():返回一个Set,其中包含了所有的key（key不允许重复）
   ...
   //values():返回一个Collection，其中包含了所有的value（value可以重复）
   ...
   ![image-20250702114420186](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250702114420186.png)

2. LinkedHashMap：保留插入顺序

3. TreeMap:按照key来排序：要求key的数据之间可以比较，可以是同类数据并且实现Comparable接口，也可以传入一个实现了Comparator接口的对象实现不同类别对象之间的比较
...
![image-20250704104619729](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250704104619729.png)

hasNext():用于判断是否还有下一个元素

next():返回下一个元素并且使迭代器后移一个元素

remove():删除掉next()方法返回的最后一个元素，注意如果不调用next方法直接remove会报错



**并发修改问题**：

当一个迭代器正在修改集合时另一个迭代器遍历集合会导致混乱，编译器会检测这种修改并且返回`ConcurrentModificationException`
...
* 简单数组类型：Arrays.sort(arr[])
* 对象数组：Arrays.sort(arr[])，要求该类实现Comparable接口，能够实现对象间的比较
* 对象数组：Arrays.sort(arr[],Comparator)，在实现了Comparator接口的类对象中实现对象之间的比较
...
class A implements Comparable<A>{
    ...
    public int compareTo (A a) {
        if(this.name.length() > a.name.length()) {
            return 1;
        }
        else if(this.name.length() < a.name.length()) {
            return -1;
        }
        else {
            return 0;
...
class CompareObj implements Comparator<Object> {
    @Override
    public int compare(Object o1, Object o2) {
        if(o1.getClass().getSimpleName().length() > o2.getClass().getSimpleName().length()) {
            return 1;
        }
        else if(o1.getClass().getSimpleName().length() < o2.getClass().getSimpleName().length()) {
            return -1;
        }
        else {
            return 0;
...
public class ObParm{
    public static void main(String argv[]){
        ...
        A[] b = {new A("aadwqdw"), new A("abc"), new A("i"), new A("abcd")};
        Arrays.sort(b);
        for(A n : b) {
        ...
        Object[] c = {new A("aadwqdw"), new ArrayList(), new String(), new Integer(0) };
        Arrays.sort(c, new CompareObj());
        for(Object h : c) {
            System.out.print(h.getClass().getSimpleName() + " ");
        }
    }
}
```
...
yield方法：将该线程的控制权交出来，将该线程放在同一优先级队列最后，然后取出同一优先级的下一个线程进行执行
...
![image-20250707104452941](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250707104452941.png)

ip地址：为互联网上的每一台设备分配的逻辑地址

端口号：一个服务器的不同端口对应不同的服务

主机名=服务器名+域名，www嗲表万维网服务器

可以通过主机名映射到ip地址，从而进行访问



###  URL类

url是统一资源定位符，用于表示互联网上的资源地址

url的组成部分：

1. 协议名：如http,https
2. 主机名：如www.baidu.com
3. 端口号
4. 路径：资源在服务器上的路径
5. 查询参数（Query）：以`?`开头，键值对形式（如`?name=John`）
6. 片段标识符（Fragment）：以`#`开头，指向资源内部锚点（如`#section1`）

构造方法：

|                        **构造方法**                        |         **适用场景**         |                    **示例**                     |
| :--------------------------------------------------------: | :--------------------------: | :---------------------------------------------: |
|                     `URL(String spec)`                     |    通过完整URL字符串创建     |   `new URL("https://example.com:8080/path")`    |
|      `URL(String protocol, String host, String file)`      | 通过协议、主机、文件路径创建 |    `new URL("http", "example.com", "/file")`    |
| `URL(String protocol, String host, int port, String file)` |        指定端口号创建        | `new URL("https", "example.com", 443, "/data")` |
|              `URL(URL context, String spec)`               |   基于基础URL解析相对路径    |       `new URL(baseUrl, "relative/path")`       |
...
![image-20250707193939269](https://raw.githubusercontent.com/hhr2449/pictureBed/main/img/image-20250707193939269.png)
...
https://api2.newsminer.net/svc/news/queryNewsList?size=15&startDate=2024-06-20&endDate=2024-08-30&words=拜登&categories=科技&page=1

这是一个获取新闻的接口，可以调整请求参数来获取不同类别的新闻信息

https://api2.newsminer.net/svc/news/queryNewsList是资源的地址
